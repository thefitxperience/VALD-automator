"""
Static trainers and branches data for Body Motions and Body Masters.
WhatsApp numbers start empty — fill them in as: "+966XXXXXXXXX"
"""

TRAINERS = {
    "Body Motions": {
        "RUH - Al Malaz": {
            "trainers": [
                {"name": "Hadir Nasr Mohamed", "whatsapp": "+966564593282"},
                {"name": "Taif Saad Alahmari", "whatsapp": "+966558889826"},
                {"name": "Areej Hajj Abdallah", "whatsapp": "+966545178434"},
                {"name": "Zouhaira Al-Omrani", "whatsapp": ""},
                {"name": "Sarah Shaabane", "whatsapp": ""},
                {"name": "Taghrid Khaled", "whatsapp": "+966506605769"},
                {"name": "Ikram Gueddes", "whatsapp": "+966548147206"},
                {"name": "Moroj Samir Abdel Hamid", "whatsapp": "+966580803191"},
                {"name": "Nadine Ghaleb", "whatsapp": "+966560695599"},
                {"name": "Abir Salloum", "whatsapp": ""},
                {"name": "Faemeh Al Seraji", "whatsapp": ""},
                {"name": "Latoya Fantisi", "whatsapp": ""},
                {"name": "Mariam Jahin", "whatsapp": ""},
                {"name": "Loel Du Bruyn", "whatsapp": ""},
                {"name": "Shayma Hanashi", "whatsapp": ""},
                {"name": "Nwabisa Liwane", "whatsapp": ""},
            ]
        },
        "RUH - Al Sahafa": {
            "trainers": [
                {"name": "Sara Said", "whatsapp": "+966554546045"},
                {"name": "Rania Manita", "whatsapp": "+966565347515"},
                {"name": "Shefa Ghazi", "whatsapp": "+966501593090"},
                {"name": "Kholoud Khaled", "whatsapp": "+966569806309"},
                {"name": "Safaa Karim", "whatsapp": "+966532694802"},
                {"name": "Maram Tlili", "whatsapp": "+966548903929"},
                {"name": "Malek Belguith", "whatsapp": "+966569049981"},
                {"name": "Manel Manasria", "whatsapp": "+966580396823"},
                {"name": "Sirine Mohamed Hamdi", "whatsapp": "+966545610337"},
                {"name": "Guesmi Kholoud", "whatsapp": "+966541862685"},
                {"name": "Abeer Al Saloum", "whatsapp": ""},
                {"name": "Nashwa Attia Elbendary", "whatsapp": ""},
            ]
        },
        "RUH - Al Aarid": {
            "trainers": [
                {"name": "Maryam Al-Qawsari", "whatsapp": ""},
                {"name": "Shaden Omar", "whatsapp": "+966562648230"},
                {"name": "Hidaya Moubarak", "whatsapp": "+966549699665"},
                {"name": "Safa Al Saggadi", "whatsapp": "+966557352147"},
                {"name": "Malak Al Zahabi", "whatsapp": "+966568841313"},
                {"name": "Chaza Ben Miled", "whatsapp": "+966560891607"},
                {"name": "Nada Mohamed Khalifa", "whatsapp": "+966504693457"},
                {"name": "Warda Al Hajji", "whatsapp": "+966567225971"},
                {"name": "Amal Al Chiboub", "whatsapp": "+966572636461"},
                {"name": "Fawz Jibran Al-Qahtani", "whatsapp": ""},
            ]
        },
        "RUH - Al Fayha": {
            "trainers": [
                {"name": "Feryel Ben Nacer", "whatsapp": "+966545915879"},
                {"name": "Al Anoud Saleh Al-Qablan", "whatsapp": "+966558359423"},
                {"name": "Khawla Saif", "whatsapp": "+966540315625"},
                {"name": "Asmaa Adel", "whatsapp": "+966547839720"},
                {"name": "Hind Al Ahmad", "whatsapp": "+966595529511"},
                {"name": "Ebtihal Al Kathiri", "whatsapp": "+966539144332"},
                {"name": "Walaa Kamel", "whatsapp": "+966531360195"},
                {"name": "Asmaa Ahmed", "whatsapp": "+966547380644"},
                {"name": "Maram Al Sousou", "whatsapp": ""},
            ]
        },
        "RUH - Al Uraija": {
            "trainers": [
                {"name": "Aya Al-Jallouli", "whatsapp": "+966561350594"},
                {"name": "Hadiya Al-Mohammad", "whatsapp": "+966549976190"},
                {"name": "Khouloud Wanna", "whatsapp": "+966531226708"},
                {"name": "Rania Bahloul", "whatsapp": "+966566796955"},
                {"name": "Sanae Hassel", "whatsapp": "+966543725895"},
                {"name": "Basnat Abdel Qawi", "whatsapp": "+966536164234"},
            ]
        },
        "RUH - Badr": {
            "trainers": [
                {"name": "Shorouq Osama Metwaly Ibrahim", "whatsapp": "+966564035734"},
                {"name": "Arwa Ahmed Baflah", "whatsapp": "+966538626465"},
                {"name": "Hadeer Ibrahim Gamal", "whatsapp": "+966502626702"},
                {"name": "Rihem Hossam", "whatsapp": "+966535729346"},
                {"name": "Sarah Ghoneim", "whatsapp": ""},
                {"name": "Al-Hanouf Al-Khurayan", "whatsapp": ""},
                {"name": "Amal Al-Bjeoui", "whatsapp": "+21628807781"},
                {"name": "Seham Hussein", "whatsapp": ""},
                {"name": "Maysaa Mowafaq Al-Aqram", "whatsapp": ""},
            ]
        },
        "RUH - Al Badia": {
            "trainers": [
                {"name": "Rajae Al-Jabouji", "whatsapp": "+966534411673"},
                {"name": "Maram Farhawi", "whatsapp": "+966553041682"},
                {"name": "Safa Radif", "whatsapp": "+966545841571"},
                {"name": "Nermin Hadi Debaya", "whatsapp": "+966542539814"},
                {"name": "Hasna AlKayyal", "whatsapp": "+966538121195"},
                {"name": "Takwa Abdelly", "whatsapp": "+966544241126"},
                {"name": "Nawres Khordeni", "whatsapp": "+966569529765"},
                {"name": "Souha Al-Bajawi", "whatsapp": ""},
                {"name": "Hanin Barrani", "whatsapp": ""},
                {"name": "Ghofran Bint Kamal Aouni", "whatsapp": ""},
            ]
        },
        "JED - Al Basateen": {
            "trainers": [
                {"name": "Iman Kouisseh", "whatsapp": "+966563393898"},
                {"name": "Raghad Al Bir Qaddar", "whatsapp": "+966533986696"},
                {"name": "Ranim Brahmi", "whatsapp": "+966542488326"},
                {"name": "Sabrine Al Amine", "whatsapp": "+966568362419"},
                {"name": "Mouna Farhat", "whatsapp": "+966546246298"},
                {"name": "Salwa Mlaiki", "whatsapp": "+966569748433"},
                {"name": "Reyouf Saad", "whatsapp": "+966560647501"},
                {"name": "Shurooq Amer", "whatsapp": "+966530435479"},
                {"name": "Maha Fayez Al Ahmari", "whatsapp": "+966560480574"},
                {"name": "Salma Jadli", "whatsapp": "+966537665190"},
                {"name": "Moutidis Ofantisi", "whatsapp": ""},
            ]
        },
        "JED - Al Faisaliyah": {
            "trainers": [
                {"name": "Wiem Dridi", "whatsapp": "+966569896504"},
                {"name": "Shourooq Al Jundi", "whatsapp": "+966564036118"},
                {"name": "Mariam Baomar", "whatsapp": "+966543700291"},
                {"name": "Ines Rezgui", "whatsapp": "+966566894215"},
                {"name": "Kholod Khaled", "whatsapp": "+966540899707"},
                {"name": "Hajer Jamal Al-Deen Khalifa Ali", "whatsapp": "+966543880262"},
                {"name": "Chourouk Sherif", "whatsapp": "+966547439677"},
                {"name": "Hala Saied", "whatsapp": "+966544680936"},
            ]
        },
        "JED - Al Naeem": {
            "trainers": [
                {"name": "Sara Mahdi", "whatsapp": "+966562226399"},
                {"name": "Siwar Al Habib", "whatsapp": ""},
                {"name": "Shayma Abbes", "whatsapp": "+966548145605"},
                {"name": "Nahla Al Hajri", "whatsapp": "+966504399059"},
                {"name": "Afraa Almasri", "whatsapp": ""},
                {"name": "Lamia Al Amer", "whatsapp": "+966557112990"},
                {"name": "Diaa Sobhi", "whatsapp": "+966500752023"},
                {"name": "Maysem Mohamed", "whatsapp": "+966544680085"},
                {"name": "Asayel Sultan", "whatsapp": "+966569592077"},
                {"name": "Nadine Shawky", "whatsapp": ""},
                {"name": "Mayssa Bin Mohamad Nasser Mejri", "whatsapp": ""},
            ]
        },
        "DMM - Al Faisaliyah": {
            "trainers": [
                {"name": "Sirine Jawadi", "whatsapp": "+966568869312"},
                {"name": "Moulouk Al Lawi", "whatsapp": "+966564028449"},
                {"name": "Rasha Hamid", "whatsapp": "+966549353606"},
                {"name": "Soulaima Hassan", "whatsapp": "+966568025073"},
                {"name": "Rahma Qays", "whatsapp": "+966564653585"},
                {"name": "Cyrine Ali", "whatsapp": "+966564028487"},
                {"name": "Al Bandari Al Harbi", "whatsapp": "+966551462726"},
                {"name": "Mirna Mostafa", "whatsapp": "+966530695513"},
                {"name": "Nourhan Hosny", "whatsapp": "+966563876382"},
                {"name": "Amira Omar", "whatsapp": ""},
                {"name": "Mariem Laamiri", "whatsapp": ""},
            ]
        },
        "DMM - Al Jalawiah": {
            "trainers": [
                {"name": "Asmaa Mahmoud", "whatsapp": "+966540826236"},
                {"name": "Basma Baza", "whatsapp": ""},
                {"name": "Mennatllah Saad", "whatsapp": ""},
                {"name": "Wyssal Ben Dhieb", "whatsapp": "+21655523491"},
                {"name": "Asmaa Ashraf", "whatsapp": "+966557468380"},
                {"name": "Chaima Gharsalli", "whatsapp": "+966569805410"},
                {"name": "Fatima Abdallah", "whatsapp": ""},
                {"name": "Emna Ben Chalbia", "whatsapp": "+966544624416"},
                {"name": "Hebatallah Ali", "whatsapp": ""},
                {"name": "Aya Maayoufi", "whatsapp": "+966562467260"},
            ]
        },
        "DMM - Al Nada": {
            "trainers": [
                {"name": "Amal Al-Fahat Ghazi Al-Ruwaili", "whatsapp": ""},
                {"name": "Chaima Boutita", "whatsapp": "+966572685488"},
                {"name": "Asmaa Shehata", "whatsapp": "+966566961228"},
                {"name": "Sarah Rasheeq", "whatsapp": "+966545084719"},
                {"name": "Nawal Akermi", "whatsapp": "+966545200626"},
            ]
        },
        "ALQ - Buraidah": {
            "trainers": [
                {"name": "Mai Mosleh", "whatsapp": ""},
                {"name": "Chayma Hannachi", "whatsapp": "+966547969281"},
                {"name": "Malath Suleiman Alshamasi", "whatsapp": "+966550454399"},
                {"name": "Anwar Ben Rhayem", "whatsapp": "+966567863703"},
                {"name": "Lulu Alzaaj", "whatsapp": "+966509643862"},
                {"name": "Raja Reeba", "whatsapp": "+966538933346"},
                {"name": "Aya Mahmoud Abed Al-Wanis", "whatsapp": "+966568061997"},
                {"name": "Ines Souissi", "whatsapp": "+966540539626"},
                {"name": "Nouran Emad Jad", "whatsapp": "+966543396136"},
                {"name": "Nourhan Ehab Amarah", "whatsapp": "+966546316528"},
                {"name": "Anfal Nasser Alhusseini", "whatsapp": "+966543348539"},
                {"name": "Rawiya Alarousi Mabrouk", "whatsapp": "+966539181074"},
                {"name": "Eilaf Mohamed Al-Fayizi", "whatsapp": "+966506670265"},
            ]
        },
        "ALQ - Unaizah": {
            "trainers": [
                {"name": "Samar Lajnef", "whatsapp": "+966501048044"},
                {"name": "Asmaa Aalaa Al-Din", "whatsapp": "+966569151304"},
                {"name": "Esraa Labib Noomani", "whatsapp": "+966544518350"},
                {"name": "Rim Chabi", "whatsapp": "+966547607911"},
                {"name": "Yosra Al-Mzoughi", "whatsapp": "+966569822058"},
                {"name": "Roua Al-Hashemi Bin Naeimah", "whatsapp": "+966547438677"},
                {"name": "Rana Mohammad Hassan Aaref", "whatsapp": "+966567987452"},
            ]
        },
        "Al Ahsaa": {
            "trainers": [
                {"name": "Samar Mohammed", "whatsapp": "+966543965258"},
                {"name": "Mona Maatouq", "whatsapp": "+966546342122"},
                {"name": "Afnan Khalid", "whatsapp": "+966530245994"},
                {"name": "Marwa Almudyni", "whatsapp": "+966581496551"},
                {"name": "Amina Mohammad Mosbeh", "whatsapp": "+966541720132"},
                {"name": "Iman Aboushosha", "whatsapp": "+966547854553"},
                {"name": "Nour Toumi", "whatsapp": "+21627367638"},
                {"name": "Sohaila Refaat", "whatsapp": ""},
                {"name": "Esraa Idris", "whatsapp": ""},
                {"name": "Omaymah Youssef", "whatsapp": ""},
                {"name": "Habiba Alghareeb", "whatsapp": ""},
            ]
        },
        "AlUla": {
            "trainers": [
                {"name": "Walaa Ahmed", "whatsapp": "+966583657570"},
                {"name": "Amal Saleh", "whatsapp": "+966503951859"},
                {"name": "Bayan Al-Anzi", "whatsapp": "+966556389193"},
                {"name": "Hanin Barrani", "whatsapp": "+966542764732"},
                {"name": "Rehab Naji Al-Nasri", "whatsapp": "+966536183209"},
                {"name": "Khouloud Suibgui", "whatsapp": "+966540473138"},
            ]
        },
        "Tabuk": {
            "trainers": [
                {"name": "Amal Saad", "whatsapp": "+966599486170"},
                {"name": "Bassant Majed Eid", "whatsapp": "+966545828493"},
                {"name": "Marwa Jendoubi", "whatsapp": "+966569283145"},
                {"name": "Sahar Mrad Berguiga", "whatsapp": "+966561185546"},
                {"name": "Ishraf Mohamed Al-Zaghdoudi", "whatsapp": "+966548034943"},
                {"name": "Nada Ahmed Mohamed", "whatsapp": "+966545946427"},
                {"name": "Rasha Abu Hassan", "whatsapp": "+966563597236"},
                {"name": "Hanane Bankroum", "whatsapp": "+966550588623"},
            ]
        },
    },

    "Body Masters": {
        "RUH - Al Malaz": {
            "trainers": [
                {"name": "Abdallah Kamal Ahmad Sayid", "whatsapp": "+966545129541"},
                {"name": "Adnan Abdallah Al Jahni", "whatsapp": "+966530543799"},
                {"name": "Adnan Abdallah Aljahny", "whatsapp": "+966530543799"},
                {"name": "Chady Taha Abdel Rahman", "whatsapp": "+966560001693"},
                {"name": "Hicham Mahboub", "whatsapp": "+966591015467"},
                {"name": "Mahmoud Bakry", "whatsapp": "+966538720905"},
                {"name": "Mohamed Elshabrawy", "whatsapp": "+966535260246"},
                {"name": "Mohaned Ahmad", "whatsapp": "+966534997045"},
                {"name": "Omar Ismael", "whatsapp": "+201147520710"},
                {"name": "Omar Saoud", "whatsapp": "+966536988846"},
                {"name": "Trainer Ismail", "whatsapp": "+966580690196"},
                {"name": "Yassine Abbich", "whatsapp": "+966543993573"},
            ]
        },
        "RUH - Al Massif": {
            "trainers": [
                {"name": "Mohamad Touri", "whatsapp": "+966551569192"},
                {"name": "Mahmoud Abdel Rahim Senussi", "whatsapp": ""},
                {"name": "Ossama Almaslout", "whatsapp": ""},
                {"name": "Amer Abid", "whatsapp": ""},
                {"name": "Mohammed Hida", "whatsapp": "+966566220411"},
                {"name": "Youssef Lekal", "whatsapp": ""},
                {"name": "Oussama Elbaz", "whatsapp": ""},
                {"name": "Alaa Fathy", "whatsapp": ""},
                {"name": "Hany Mohamed Yousif Elhadad", "whatsapp": "+966557975824"},
                {"name": "Sleiman Al Tayyar", "whatsapp": ""},
                {"name": "Asmah Choukeni", "whatsapp": ""},
                {"name": "Saleh Al Abed", "whatsapp": "+966563316506"},
                {"name": "Ahmed Shoeeb", "whatsapp": "+966540820361"},
                {"name": "Anouar Aghmour", "whatsapp": "+966564936738"},
                {"name": "Mohammad Souud", "whatsapp": ""},
                {"name": "Ahmed Keshk", "whatsapp": "+966546582827"},
                {"name": "Jayson Atlano", "whatsapp": ""},
                {"name": "Mahmoud Abdel Rahim Senussi Kame", "whatsapp": ""},
            ]
        },
        "RUH - Al Aarid": {
            "trainers": [
                {"name": "Mhamad El Ebri", "whatsapp": ""},
                {"name": "Chehab Al Dine Al Salihi", "whatsapp": ""},
                {"name": "Mahmoud Abed El Rahim", "whatsapp": "+966562436370"},
                {"name": "Captain Housein Ali Joumaa", "whatsapp": ""},
                {"name": "Diyaa Al Jraydan", "whatsapp": ""},
                {"name": "Mourad Belgacem", "whatsapp": "+966590307296"},
                {"name": "Mahmoud Yosri Mahmoud Mahfouz", "whatsapp": "+966594884676"},
                {"name": "Amr El Sayed Abdelhamid", "whatsapp": ""},
                {"name": "Moustafa Nasser Abd Elkader", "whatsapp": "+966502632480"},
                {"name": "Khalid Mouazen", "whatsapp": ""},
            ]
        },
        "RUH - Al Sahafa": {
            "trainers": [
                {"name": "Mohamad Fouad G. Amer", "whatsapp": ""},
                {"name": "Ahmad H. Abed Al Latif", "whatsapp": ""},
                {"name": "Hisham M. Badih", "whatsapp": "+966541146803"},
                {"name": "Diya Y. Aljirdan", "whatsapp": ""},
                {"name": "Mohamad Zakariah", "whatsapp": "+966560448139"},
                {"name": "Rrnniel Villaverde", "whatsapp": ""},
                {"name": "Magomed Ansarov", "whatsapp": ""},
                {"name": "Zakaria Jawadi", "whatsapp": ""},
                {"name": "Renelle Vilaverdi", "whatsapp": ""},
                {"name": "Fawzi Baltaji", "whatsapp": ""},
                {"name": "Akram M. Hussin", "whatsapp": ""},
                {"name": "Zakaria Yahia", "whatsapp": ""},
                {"name": "Youssef Noreddine", "whatsapp": ""},
                {"name": "Ahmad Fadel", "whatsapp": ""},
                {"name": "Rachid Sakouti", "whatsapp": ""},
                {"name": "Hamza Sanbar", "whatsapp": ""},
                {"name": "Ayman N. Al Chahawi", "whatsapp": "+966563022909"},
            ]
        },
        "RUH - Al Wadi": {
            "trainers": [
                {"name": "Ahmed Hamid Abdellatif Nogalla", "whatsapp": ""},
                {"name": "Bander Albalawi", "whatsapp": ""},
                {"name": "El Kaddioui Adil", "whatsapp": ""},
                {"name": "Shamsudheen Paramban", "whatsapp": ""},
                {"name": "Omar Mayhoub", "whatsapp": "+966599011997"},
                {"name": "Ibrahim Mohamed Fadel Abduljalil Barakat", "whatsapp": ""},
                {"name": "Abdellatif Zamzim", "whatsapp": ""},
                {"name": "Abdou Wahed Zraidi", "whatsapp": "+966500386796"},
            ]
        },
        "RUH - Eshbilia": {
            "trainers": [
                {"name": "Bilel Azhar Yakoubi", "whatsapp": "+966560788309"},
                {"name": "Houssam Ali Elsheshtawy", "whatsapp": "+966547804429"},
                {"name": "Marwan Al-Jebali", "whatsapp": "+966562168909"},
                {"name": "Mohamed Ibrahim Ibrahim Abouelrous", "whatsapp": "+966598628082"},
                {"name": "Ibrahim Mostafa", "whatsapp": "+966541688383"},
                {"name": "Mohamd Refai Mohamd", "whatsapp": ""},
                {"name": "Saud Turki Alqasir", "whatsapp": "+966537575773"},
            ]
        },
        "RUH - Muzahmiyah": {
            "trainers": [
                {"name": "Khalifa El Nahrawy", "whatsapp": "+966580808807"},
                {"name": "Abdssamad Aballa", "whatsapp": "+966561843132"},
                {"name": "Nabil Essahraoui", "whatsapp": "+966533423481"},
            ]
        },
        "RUH - Rabwa": {
            "trainers": [
                {"name": "Aref Hebatala", "whatsapp": "+966532184869"},
                {"name": "Alaa Eddine Rtili", "whatsapp": "+966533606204"},
                {"name": "Abderrazak Sihabi", "whatsapp": "+966544260448"},
                {"name": "Mostafa Ashour Elkhouly", "whatsapp": "+966542674377"},
            ]
        },
        "RUH - Salam": {
            "trainers": [
                {"name": "Hicham Mouhime", "whatsapp": "+966594917442"},
                {"name": "Abdellatif Zamzam", "whatsapp": "+966561757224"},
                {"name": "Fady Elsayed", "whatsapp": "+966581390049"},
                {"name": "Mohamed Kamal", "whatsapp": "+966561537676"},
            ]
        },
        "RUH - Swaidi": {
            "trainers": [
                {"name": "Nasser Mansour Nasser Alessa", "whatsapp": "+966555712965"},
                {"name": "Mohammed Nshbat", "whatsapp": "+966557736676"},
                {"name": "Naoufel Jabnouni", "whatsapp": "+966563263180"},
                {"name": "Khalid Chatir", "whatsapp": "+966500944133"},
                {"name": "Mohammed Korchi", "whatsapp": "+966503803175"},
                {"name": "Ahmed Elsayed", "whatsapp": "+966547401898"},
            ]
        },
        "RUH - Takhasousi": {
            "trainers": [
                {"name": "Abdenabi Ktraouach", "whatsapp": "+966565298423"},
                {"name": "Fikri Acharif", "whatsapp": "+966552611090"},
                {"name": "Ahmed Abdelrahman", "whatsapp": "+966564548793"},
                {"name": "Salman Altayyar", "whatsapp": "+966552181924"},
                {"name": "Wasem Seed Alhajree", "whatsapp": "+966534060008"},
                {"name": "Khalid Nokracchi", "whatsapp": "+966540056888"},
                {"name": "Taib Jadia", "whatsapp": "+966506793930"},
                {"name": "Mahmoud Nagib", "whatsapp": "+966537830310"},
                {"name": "Saeed Al Adrham", "whatsapp": "+966535356013"},
                {"name": "Ahmed Adel Abdellatif Farag", "whatsapp": ""},
            ]
        },
        "RUH - Al Badia": {
            "trainers": [
                {"name": "Abderrazak Sihabi", "whatsapp": ""},
                {"name": "Adil Elqady", "whatsapp": "+966545172553"},
                {"name": "Adil Chahmi", "whatsapp": "+966580530664"},
                {"name": "Amine Korchi", "whatsapp": "+966506261333"},
                {"name": "El Mehdi Adnani", "whatsapp": "+966545355183"},
                {"name": "Mohamed Saeed", "whatsapp": "+966563264508"},
                {"name": "Selim Ben Abdelkader", "whatsapp": "+966562346963"},
                {"name": "Mahmoud Fotoh", "whatsapp": ""},
                {"name": "Khalifa Hassan Khalifa Nahrawy", "whatsapp": "+966580808807"},
                {"name": "Mohamad Mahdim Alqahtani", "whatsapp": "+966556718174"},
            ]
        },
        "RUH - Al Fayha": {
            "trainers": [
                {"name": "Faraj Lakaira", "whatsapp": "+966551838742"},
                {"name": "Mahmoud Al Mahdi", "whatsapp": "+966550984911"},
                {"name": "Ahmad Hamdi Knawi", "whatsapp": "+966591850202"},
                {"name": "Kamal Al-Rayes", "whatsapp": "+966506095878"},
                {"name": "Asser Al-Sheikh", "whatsapp": "+966562383415"},
                {"name": "Hamza Aissaoui", "whatsapp": "+966543292026"},
                {"name": "Hicham Mouhime", "whatsapp": "+966594857442"},
                {"name": "Ibrahim Pokoti", "whatsapp": "+966592696843"},
            ]
        },
        "RUH - Al Khaleej": {
            "trainers": [
                {"name": "Ahmad Ramadan", "whatsapp": "+966501671379"},
                {"name": "Chouaib Suilmi", "whatsapp": "+966509392380"},
                {"name": "Elsayed Fouad", "whatsapp": "+966545137704"},
                {"name": "Karim Hosny Ahmad", "whatsapp": ""},
                {"name": "Mohamed Hassan Ali Hassan", "whatsapp": "+966543091194"},
                {"name": "Mohamed Moustafa Ahmad", "whatsapp": "+966547819642"},
                {"name": "Sameh Houssein Ali Abed Al-Wahid", "whatsapp": "+966500478919"},
                {"name": "Oussama Ait Hammou Obrahim", "whatsapp": "+966541872479"},
                {"name": "Zakaria Elouali", "whatsapp": "+966569596598"},
            ]
        },
        "RUH - Al Kharj": {
            "trainers": [
                {"name": "Mahmoud Mohamed", "whatsapp": ""},
                {"name": "Mohamed Hamdan Mohamed", "whatsapp": "+966558370880"},
                {"name": "Adel Ali Mohamed", "whatsapp": "+966594603016"},
                {"name": "Boujemaa Elakkad", "whatsapp": "+966501318459"},
                {"name": "Nabil Essahraoui", "whatsapp": ""},
                {"name": "Yassine El Barnoussi", "whatsapp": "+966504361531"},
            ]
        },
        "RUH - Al Nahda": {
            "trainers": [
                {"name": "Achraf Fadlallah", "whatsapp": "+966543027275"},
                {"name": "Raqan Abdel Mehsen Al-Qahtani", "whatsapp": "+966550284472"},
                {"name": "Abed El Hakim Sahou", "whatsapp": "+966556980548"},
                {"name": "Majid Abdessamad", "whatsapp": "+966503253070"},
                {"name": "Mhamed Oulahssine", "whatsapp": "+966560798022"},
                {"name": "Ramy Alla Tawfeak Ahmed", "whatsapp": "+966545322081"},
                {"name": "Rhey Anthony T. Munez", "whatsapp": "+966500251308"},
                {"name": "Youssef Al Habib Trabelsi", "whatsapp": "+966537448486"},
                {"name": "Hicham Toualbi", "whatsapp": "+966547058178"},
                {"name": "Soufian Bija", "whatsapp": ""},
            ]
        },
        "RUH - Badr": {
            "trainers": [
                {"name": "Adawi Abdelmounnmadawi", "whatsapp": ""},
                {"name": "Kamel Mohammed Kamel", "whatsapp": "+966541640458"},
                {"name": "Khalled Saeed Saeed Alhasany", "whatsapp": ""},
                {"name": "Mahmoud Farouk Abdel Karim Badawi", "whatsapp": "+966534337685"},
                {"name": "Mohammed Bstan Abd Aljwad", "whatsapp": "+966594492011"},
                {"name": "Mohammed Mubarak Mohammed Aldawsari", "whatsapp": ""},
            ]
        },
        "RUH - Ezdehar": {
            "trainers": [
                {"name": "Ahmed Maher Ahmed Nasrelden", "whatsapp": "+966541201190"},
                {"name": "Hesham Rady Ismail Rady", "whatsapp": "+966543454143"},
                {"name": "Mohsen Hadikhamis", "whatsapp": "+966559292910"},
                {"name": "Tarik Maarouf", "whatsapp": "+966580464502"},
                {"name": "Ibrahim Ali Ismaiel Ibrahim", "whatsapp": "+966568806323"},
                {"name": "Wajih Kamal Matoussi", "whatsapp": "+966564496193"},
            ]
        },
        "RUH - Murooj": {
            "trainers": [
                {"name": "Abdelaziz El Maydy", "whatsapp": "+966542101523"},
                {"name": "Amr Ahmed Abdellatif Rabia", "whatsapp": "+966532172357"},
                {"name": "Cherif Bin Ahmed Khlifi", "whatsapp": "+966530440204"},
                {"name": "Meshari Saad Alharbi", "whatsapp": ""},
            ]
        },
        "RUH - Shubra": {
            "trainers": [
                {"name": "Abdessamade Moudakkir", "whatsapp": "+966540041146"},
                {"name": "Ahmed Maher Elsayed Abdelaal", "whatsapp": "+966597515259"},
                {"name": "Hicham Oulias", "whatsapp": "+966537897800"},
                {"name": "Mohammed Abdullah Alquwaya", "whatsapp": "+966533406629"},
                {"name": "Mohammed Mahdi Adham Nabhan", "whatsapp": "+966557400074"},
                {"name": "Osman Nagi Osman", "whatsapp": "+966507579032"},
                {"name": "Rachid Choukhmane", "whatsapp": "+966502619096"},
                {"name": "Tariq Abdulrahman Basharahil", "whatsapp": "+966566441212"},
            ]
        },
        "DMM - Al Athir": {
            "trainers": [
                {"name": "Abdullah Abdulrahman", "whatsapp": "+966581203193"},
                {"name": "Abdulrahman Ali", "whatsapp": ""},
                {"name": "Ibrahim Salama", "whatsapp": "+966532439753"},
                {"name": "Rashid Elbouazizi", "whatsapp": "+966558648546"},
                {"name": "Mohamed Elsayed Elzeftawy", "whatsapp": "+966545109222"},
                {"name": "Alaa El Din Mekhtar", "whatsapp": ""},
                {"name": "Ammar Ali", "whatsapp": ""},
                {"name": "Jamal Halabi Assiri", "whatsapp": "+966534907900"},
            ]
        },
        "DMM - Al Jameyeen": {
            "trainers": [
                {"name": "Nader Maayouf Saleh", "whatsapp": "+966548047548"},
                {"name": "Hesham Adel Almarshoud", "whatsapp": "+966544643566"},
                {"name": "Mohamed Khalid Manni", "whatsapp": ""},
                {"name": "Mohamed Abdelsalam Deyab", "whatsapp": "+966558182982"},
                {"name": "Hamza El Moumine", "whatsapp": ""},
                {"name": "Mohamed Araby", "whatsapp": ""},
                {"name": "Shahid Elkhalfy", "whatsapp": "+966507337412"},
                {"name": "Mohammed Abdelhamid", "whatsapp": "+966572374523"},
                {"name": "Ashraf Ezzeldine", "whatsapp": ""},
                {"name": "Ibrahim Babaker", "whatsapp": ""},
                {"name": "Mohamad Ahmad", "whatsapp": ""},
                {"name": "Ali Ahmed Alkhadhir", "whatsapp": ""},
            ]
        },
        "DMM - Hufof": {
            "trainers": [
                {"name": "Ahmad Abdelgafour Omar Ahmad", "whatsapp": "+966536019805"},
                {"name": "Abdelrahman Mohamed", "whatsapp": "+966598042055"},
                {"name": "Khalid Karmou", "whatsapp": "+966565250428"},
                {"name": "Abdul Raouf Mustafa", "whatsapp": "+966547804344"},
            ]
        },
        "DMM - Khobar": {
            "trainers": [
                {"name": "Radwan Mohamed Radwan", "whatsapp": "+966534946448"},
                {"name": "Abbas Okasha", "whatsapp": "+966546570224"},
                {"name": "Khalid Mahmoud Saleh", "whatsapp": "+966505676495"},
                {"name": "Abed AlWasi", "whatsapp": "+966580255114"},
                {"name": "Mahmoud Hassan", "whatsapp": ""},
                {"name": "Shehab Ashraf Mohamady Ibrahim", "whatsapp": "+966547817964"},
                {"name": "Ayoub Chatat", "whatsapp": ""},
                {"name": "Ahmad Al Dokhi", "whatsapp": ""},
                {"name": "Joesel Bodota Casimiro", "whatsapp": "+966554587446"},
                {"name": "Mohamad Maani", "whatsapp": ""},
                {"name": "Rabana Gabif", "whatsapp": ""},
            ]
        },
        "JED - Hamadania": {
            "trainers": [
                {"name": "Abd Elramhan Mokhtar", "whatsapp": "+966559995578"},
                {"name": "Abdullah Mohammed Alhamdni", "whatsapp": "+966548911731"},
                {"name": "Mohamed Ibrahim Mohamed", "whatsapp": "+966593887197"},
                {"name": "Musa Eltom Ahmed", "whatsapp": "+966557535912"},
                {"name": "Hassan Bargougui", "whatsapp": "+966560941885"},
            ]
        },
        "JED - Al Rawdah": {
            "trainers": [
                {"name": "Abdallah Kandaly", "whatsapp": "+966532200399"},
                {"name": "Ibrahim Nasr", "whatsapp": "+966591638245"},
                {"name": "Khaled Al-Masouri", "whatsapp": "+966532394494"},
                {"name": "Sami Serrar", "whatsapp": "+966592586391"},
                {"name": "Sofiane Sadiq", "whatsapp": "+966507607687"},
                {"name": "Tawfik Jukh", "whatsapp": "+966537457842"},
                {"name": "Youssef Souayah", "whatsapp": "+966500145773"},
                {"name": "Muhmoud Ayman Saad", "whatsapp": "+966583840084"},
                {"name": "Ahmed Elsayed Algammal", "whatsapp": ""},
                {"name": "Moez Mustafa", "whatsapp": "+966598360035"},
                {"name": "Soliman Bakar", "whatsapp": "+966544201272"},
                {"name": "Mohamed Fathi Abdulsattar", "whatsapp": ""},
                {"name": "Oussama Amzil", "whatsapp": ""},
            ]
        },
        "JED - Makkah": {
            "trainers": [
                {"name": "Adel Mohammed Abdullah Al-Daos", "whatsapp": ""},
                {"name": "Imad Al-Zahrani", "whatsapp": ""},
                {"name": "Khaled Al Barkawi", "whatsapp": "+966583885346"},
                {"name": "Mahmoud Al Bihery", "whatsapp": "+966547803835"},
                {"name": "Mohamed Ait Ayad", "whatsapp": "+966565026095"},
                {"name": "Mohamed Gamea", "whatsapp": "+966598316907"},
                {"name": "Omar Al Shankety", "whatsapp": "+966543687234"},
            ]
        },
        "JED - Obhor - Al Amwaj": {
            "trainers": [
                {"name": "Hesham Galal Mahmoud", "whatsapp": "+966581465161"},
                {"name": "Karim Abdala Wassal", "whatsapp": "+966553284707"},
                {"name": "Mohammed Joharji", "whatsapp": "+966563274581"},
                {"name": "Mohammed Mustafa Himsi", "whatsapp": "+966544487711"},
            ]
        },
        "JED - Obhor - Al Sheraa": {
            "trainers": [
                {"name": "Mohamad Rida Irabi", "whatsapp": "+966569039676"},
                {"name": "Badereddine Bouzahar", "whatsapp": "+966540327695"},
                {"name": "Abed Al Menhem Mhamad", "whatsapp": "+966561499730"},
                {"name": "Ayoub Benzahra", "whatsapp": "+966549134851"},
                {"name": "Ahmed Al Gammal", "whatsapp": "+966578982567"},
                {"name": "Bader Eddine", "whatsapp": "+966540327695"},
                {"name": "Abdelrahman Kamal", "whatsapp": "+966502691628"},
                {"name": "Tarak Othman", "whatsapp": "+966594117809"},
                {"name": "Mostafa Hamada", "whatsapp": "+966535701729"},
                {"name": "Nawaf Alanazi", "whatsapp": "+966581632360"},
                {"name": "Ibrahim Saleh", "whatsapp": "+966531810551"},
                {"name": "Abdelrahman Faris", "whatsapp": "+966549158644"},
                {"name": "Abed El Rahman Taha", "whatsapp": "+966532455054"},
                {"name": "Mahmoud Fawzy", "whatsapp": "+966557197298"},
            ]
        },
        "ALQ - Al Rass": {
            "trainers": [
                {"name": "Yousef Bou Kentar", "whatsapp": "+966582142826"},
                {"name": "Hicham Hemsi", "whatsapp": ""},
            ]
        },
        "ALQ - Buraidah": {
            "trainers": [
                {"name": "Oussema Boussaid", "whatsapp": "+966548668947"},
                {"name": "Mouad Omir", "whatsapp": ""},
                {"name": "Orley Garcia", "whatsapp": ""},
                {"name": "Yasser Mahmoud", "whatsapp": "+966538502337"},
                {"name": "Kamis Zamzam", "whatsapp": ""},
            ]
        },
        "ALQ - Unaizah": {
            "trainers": [
                {"name": "Mohamed Jaouat", "whatsapp": ""},
                {"name": "Mohamed Amzil", "whatsapp": "+966550132300"},
                {"name": "Mohamed Othman Gazy", "whatsapp": ""},
                {"name": "Karim Maarori", "whatsapp": ""},
                {"name": "Ahmed Tarek Kholafa", "whatsapp": ""},
            ]
        },
        "MED - Shouran": {
            "trainers": [
                {"name": "Abdel Ghafour Bahja", "whatsapp": "+966568114364"},
                {"name": "Anas Mohmmed Alshanqiti", "whatsapp": "+966501598733"},
                {"name": "Lakhdari Abdellatif", "whatsapp": "+966564901566"},
                {"name": "Mohamed Soliman Ragab", "whatsapp": "+966596708776"},
                {"name": "Sheref Ashraf Mohamed", "whatsapp": "+966538849138"},
                {"name": "Ahmed Mohamed Helmy Tohamy", "whatsapp": ""},
                {"name": "Adham Ahmed Abdelsttar", "whatsapp": ""},
                {"name": "Ahmed Abdallah Elsayed Ibrahim", "whatsapp": ""},
                {"name": "Hassan Adel Almughamisi", "whatsapp": ""},
                {"name": "Amjad Hussein Abbs Abdulaal", "whatsapp": ""},
            ]
        },
        "MED - Taiba": {
            "trainers": [
                {"name": "Abdelaziz Mohi Eldin Elkhouli", "whatsapp": "+966503930091"},
                {"name": "Abdulwahab Ahmed Saad Al Ghamdi", "whatsapp": "+966501663031"},
                {"name": "Amgad Hussein Abbas Abdulaal", "whatsapp": "+966551356507"},
                {"name": "Mostafa Mahmoud Sayed Mohamed", "whatsapp": "+966504372358"},
                {"name": "Rabah Salamah Alhujuri", "whatsapp": "+966599831400"},
            ]
        },
        "Uhud": {
            "trainers": [
                {"name": "Abdallah Mohamed Ali", "whatsapp": "+966548992386"},
                {"name": "Ahmad Al-Dawkhi", "whatsapp": ""},
                {"name": "Hassan Ait Hmida", "whatsapp": "+966543880262"},
                {"name": "Hamza Berahhou", "whatsapp": ""},
                {"name": "Moulay Al Hussein", "whatsapp": "+966558422010"},
                {"name": "Wael Mohammed", "whatsapp": ""},
                {"name": "Hussein Elsayed", "whatsapp": "+966536212468"},
                {"name": "Anas Al-Makenzi", "whatsapp": "+966566249290"},
                {"name": "Mohamed Rabhi", "whatsapp": "+966598286282"},
                {"name": "Enrique Jr Ravallo-Macorol", "whatsapp": ""},
                {"name": "Mohamed Abdellatif Radwan", "whatsapp": ""},
                {"name": "Hamza Said", "whatsapp": ""},
                {"name": "Hicham Hassi", "whatsapp": ""},
                {"name": "Mohamed Abdel Moeti", "whatsapp": "+966500780502"},
                {"name": "Houssem Saggedi", "whatsapp": ""},
            ]
        },
        "AlUla": {
            "trainers": [
                {"name": "Ahmed Abdelmaqsoud", "whatsapp": "+966550105380"},
                {"name": "Ahmed Khaled", "whatsapp": "+966539211836"},
                {"name": "Ahmed Khaled El Said", "whatsapp": "+966561786327"},
                {"name": "ElHassan Nagib", "whatsapp": "+966545935469"},
                {"name": "Mohamad Amine Azhar", "whatsapp": ""},
                {"name": "Mohannad Hani", "whatsapp": "+966551564841"},
                {"name": "Moamed Ata", "whatsapp": "+966566315114"},
                {"name": "Yassin Mbarek", "whatsapp": "+966535813394"},
            ]
        },
        "Al Mubaraz": {
            "trainers": [
                {"name": "Abdallah Lazam Faisal Al-Asadi", "whatsapp": "+966569778479"},
                {"name": "Hamza Moustaghfir", "whatsapp": "+966542739204"},
                {"name": "Mohamed Fouad Amer", "whatsapp": "+966548053519"},
                {"name": "Ammar Tarek Fathi Ibrahim Ali Mahmoud", "whatsapp": ""},
                {"name": "Radi Gamal Osman Hussein", "whatsapp": "+966530546638"},
                {"name": "Sabry Ali Abdelmonem Ezzah", "whatsapp": "+966595271356"},
                {"name": "Samer Mohammed Abas Kamber", "whatsapp": "+966581195121"},
                {"name": "Khaled Ezzat", "whatsapp": "+966580642326"},
                {"name": "Abd Elrahem Al Ouina", "whatsapp": "+966538036523"},
                {"name": "Amar Fathi", "whatsapp": ""},
            ]
        },
        "Hafr El Batin": {
            "trainers": [
                {"name": "Ahmed Saad Elmamlouk", "whatsapp": "+966570890095"},
                {"name": "Mohamed Abdel Manaam Yakout", "whatsapp": ""},
                {"name": "Mohamed Achraf Abed Al-Mawjoud Mohamed", "whatsapp": ""},
                {"name": "Mohamed Ibrahim Abdelaal", "whatsapp": "+966593887197"},
                {"name": "Mahmoud El-Sayed Abdelwareth Radwan", "whatsapp": "+966568601329"},
                {"name": "Mohamad Ashraf", "whatsapp": ""},
                {"name": "Abdel Rahman Hamad", "whatsapp": ""},
                {"name": "Ahmad Al Hussein", "whatsapp": ""},
            ]
        },
        "Tabuk": {
            "trainers": [
                {"name": "Miloud Baali", "whatsapp": ""},
                {"name": "Mohammad Nour Al-Din Awad Mohammad", "whatsapp": "+966550549740"},
                {"name": "Mohammad Ismail", "whatsapp": ""},
                {"name": "Mostafa Hamada", "whatsapp": ""},
                {"name": "Anas Jouhari", "whatsapp": ""},
                {"name": "Tamer Sharkas", "whatsapp": "+966540839375"},
            ]
        },
        "Najran": {
            "trainers": [
                {"name": "Abdaleleh Naser Ahmed", "whatsapp": ""},
                {"name": "Ibrahim Refat Abdellatif", "whatsapp": "+966594808496"},
                {"name": "Wadhah Ahmed Ahmed Mahyub", "whatsapp": ""},
                {"name": "Ahmed Mohamed Ahmed Fadol", "whatsapp": "+966508607985"},
                {"name": "Said Ennasr", "whatsapp": "+966549057001"},
            ]
        },
        "Khamis Mushait": {
            "trainers": [
                {"name": "Amin Haron", "whatsapp": ""},
                {"name": "Eslam Rezk", "whatsapp": "+966543208793"},
                {"name": "Hamdy Helal", "whatsapp": ""},
                {"name": "Ali Alkobebi", "whatsapp": "+966555244760"},
                {"name": "Ibrahem Zahran", "whatsapp": "+966540011286"},
                {"name": "Mustapha Selaoui", "whatsapp": "+966565038209"},
                {"name": "Mohammed Ayed", "whatsapp": ""},
            ]
        },
        "Hail": {
            "trainers": [
                {"name": "Abdallah Laghzal", "whatsapp": ""},
                {"name": "Ahmad Chiboub", "whatsapp": ""},
                {"name": "Ali Mohamed Ali Mohamed", "whatsapp": ""},
                {"name": "Bassam Ahmad Al-Salhani", "whatsapp": ""},
                {"name": "Hatem Kamal Hatiba", "whatsapp": ""},
                {"name": "Wagih Al-Saadani", "whatsapp": ""},
            ]
        },
    }
}


def get_branches(gym: str) -> list[str]:
    return list(TRAINERS.get(gym, {}).keys())


def get_trainers(gym: str, branch: str) -> list[dict]:
    return TRAINERS.get(gym, {}).get(branch, {}).get("trainers", [])


def get_trainer_whatsapp(gym: str, branch: str, trainer_name: str) -> str:
    for t in get_trainers(gym, branch):
        if t["name"] == trainer_name:
            return t.get("whatsapp", "")
    return ""
